...
import { ChevronLeft, Edit, Home, Star, FileText, MapPin, MessageSquare, Users, Shield, X, ChevronRight, Plus } from "lucide-react"
import { useParams, useRouter } from "next/navigation"
import { PropertyOwnerDetails } from "./property-owner-details"
import { PropertyManagerInfo } from "./property-manager-info"
import { BackNavButton } from "../../backNavButton"
import { Manager } from "@/libs/interface"
...
export default function PropertyValueDetails({ isMobile = false }: PropertyDetailsProps) {
	...
	const handleAssignManager = (manager: Manager) => {
		setCurrentManager(manager)
		console.log("Assigned manager:", manager)
	}
	const actionButtons = [
		// { id: `/admin/properties/material-cost/${property_id}`, label: "Building Material Cost", icon: "Home" },
		{ id: `#`, label: "Building Material Cost", icon: "Home" },
		// { id: `/admin/properties/timeline/${property_id}/expert-review`, label: "Expert Review", icon: "Star" },
		{ id: `#`, label: "Expert Review", icon: "Star" },
		// { id: `/admin/properties/timeline/${property_id}/policy-updates`, label: "State Policy Updates", icon: "FileText" },
		{ id: `#`, label: "State Policy Updates", icon: "FileText" },
		// { id: `/admin/properties/landmarks/${property_id}`, label: "Landmarks", icon: "MapPin" },
		{ id: `#`, label: "Landmarks", icon: "MapPin" },
		// { id: `/admin/properties/timeline/${property_id}/word-on-street`, label: "Word on the Street", icon: "MessageSquare" },
		{ id: `#`, label: "Word on the Street", icon: "MessageSquare" },
		// { id: `/admin/properties/timeline/${property_id}/community-update`, label: "Community update", icon: "Users" },
		{ id: `#`, label: "Community update", icon: "Users" },
		// { id: `/admin/properties/vulnerability-test/${property_id}`, label: "Request Vulnerability Test", icon: "Shield" },
		{ id: `#`, label: "Request Vulnerability Test", icon: "Shield" },
	]
	...
	const handleEditProperty = () => {
		router.push("/admin/properties/new-property")
	}
				...
				<div className="p-4 space-y-4">
					...
					<div className="flex flex-col gap-4 py-6 px-4 rounded-2xl min-h-[10.25rem] border border-[#E9EAEB] bg-[#FFFFFF]">
						...
						<PropertyManagerInfo
							...
							onAssignProperty={() => setIsAssignModalOpen(true)}
						/>
					</div>
					...
					<div className="bg-white rounded-2xl shadow-lg shadow-[#1E1E1E05] border border-[#E9EAEB] p-6">
						...
						<PropertyOwnerDetails owner={propertyOwner} />
					</div>
				</div>
				{showImageModal && (
			...
			<header className="px-4 md:px-6 py-4">
				<div className="flex items-center justify-between">
						...
						<div className="flex flex-col md:flex-row md:items-center justify-between">
							<div className="flex items-center gap-4 mb-4 md:mb-0">
								...
								<nav className="text-sm text-[#7E838D]">
									...
									<span className="mx-2">/</span>
									<span className="max-w-[80px] truncate inline-block align-bottom text-[#212A3B]" title="Estimated Property Value" >Estimated Property Value.</span>
								</nav>
							</div>
						</div>
					</div>
					<div className="flex gap-4">
						<button
							onClick={handleEditProperty}
							className="flex items-center gap-2 px-4 py-2 bg-[#ECF4F1] text-[#474E5C] rounded-lg transition-colors cursor-pointer"
						>
			...
			<div className="max-w-7xl mx-auto p-4 md:p-6">
				<div className="grid gap-6 grid-cols-5">
					...
					<div className="col-span-2 flex	flex-col gap-6">
						...
						{/* Estimated Property Value */}
						<div className="relative rounded-lg shadow-sm money-card-wrapper bg-[#ECF4F1] h-[8.75rem]">
							<div className="p-6 money-card relative flex flex-col gap-6">
							...
							<button
								onClick={handleEditProperty}
								className="cursor-pointer absolute top-4 right-4 w-10 h-10 bg-[#fff] text-[#474E5C] rounded-lg flex items-center justify-center transition-colors"
							>
								<Edit className="w-5 h-5" />
							</button>
						</div>
