...
export default function ExpertReview({ isMobile = false }: ExpertReviewProps) {
	...
	if (isMobile) {
		return (
			<div className="min-h-screen bg-gray-50">
									<OverViewBreadCrumbs property={"GreenLand Estate"} page={"Expert Review"} />
				...
				<div className="flex flex-col gap-4">
					...
					<div className="bg-white rounded-lg shadow-sm p-4">
						<div className="flex items-center justify-between mb-6">
							<h2 className="text-base font-medium text-[#212A3B]">Review</h2>
							<button
							...
							{expertReviews.map((review) => (
								<div key={review.id} className="flex gap-4">
									...
									<div className="flex flex-col items-center">
										<div className={`w-4 h-4 border-4 ${review.done ? "bg-[#448C74] border-[#ECF4F1]" : " bg-[#A4A8AF] border-[#FFFFFF]"} rounded-full flex-shrink-0 mt-1`} />
										{review.id !== expertReviews[expertReviews.length - 1].id && (
									...
									<div className="flex-1 pb-6">
										<p className="text-sm text-gray-500 mb-2">{review.timestamp}</p>
		...
		<div className="min-h-screen flex flex-col gap-6">
			...
			<div className="flex flex-col gap-4">
				...
				<div className="bg-white rounded-lg shadow-sm p-6">
					<div className="flex items-center justify-between mb-6">
						<h2 className="text-base font-medium text-[#212A3B]">Expert Review</h2>
						<button
							onClick={handleFiltersClick}
							className="flex items-center gap-2 px-4 py-2"
						>
					...
					<div className="space-y-8">
						{expertReviews.map((review) => (
							<div key={review.id} className="flex gap-4">
								...
								{/* Review Content */}
								<div className="flex-1 pb-6">
									<p className="text-sm text-gray-500 mb-3">{review.timestamp}</p>
