...
for (let n = 0; n < borderRadiusElements.length; n++)
  ++++(borderRadiusElements[n].value = getConfigFromStorage("theme-" + borderRadiusElements[n].id + "multiplier") || borderRadiusElements[n].value),
    ++++borderRadiusElements[n].addEventListener("input", (e) => {
      ...
      ++++++++updateStorageConfig("theme-" + e.target.id, "calc(var(--ar-border-radius) * " + n + ")"), updateStorageConfig("theme-" + e.target.id + "multiplier", n), styleSheetRules.root.style.setProperty("--" + prefix + e.target.id, "calc(var(--ar-border-radius) * " + n + ")"), appendStylesToPreElement();
      ++++  });
